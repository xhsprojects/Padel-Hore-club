...
export default function PlayerProfilePage() {
  ...
  // Get matches for the viewed player
  ...
  const isLoading = playerLoading || matchesLoading || allPlayersLoading || isUserLoading || isCurrentUserProfileLoading || seasonsLoading || tierSettingsLoading;
  ...
  }, [isLoading, user, currentUserProfile, router]);
  ...
  const isAdmin = currentUserProfile?.role === 'admin' || isDefaultAdmin;
  const isMember = currentUserProfile?.role === 'member';
  const isOwner = user?.uid === id;
  const canView = isAdmin || isMember || isOwner;
          ...
          <div className="p-4 sm:p-6 lg:p-8 text-center">
              <Card className="max-w-md mx-auto">
                  <CardHeader>
                      ...
                      <CardTitle>Fitur Khusus Member</CardTitle>
                      <CardDescription>Anda harus menjadi member untuk melihat detail profil pemain lain.</CardDescription>
                  </CardHeader>
                  <CardContent>
                      <p className="text-sm text-muted-foreground mb-4">
                          Upgrade keanggotaan Anda untuk membuka akses ke statistik pemain, riwayat pertandingan, dan lainnya!
                      </p>
                      <Button onClick={() => router.push('/membership')}>Lihat Keuntungan Member</Button>
                  </CardContent>
              </Card>
          </div>
      );
  ...
  const canSeeWhatsapp = player.showWhatsapp === true || isAdmin || isOwner;
                ...
                <TooltipProvider>
                    <div className="pr-4 space-y-4">
                        {sortedMatches.map(match => {
                            ...
                            const myPartnerId = myTeamIds.find(pId => pId !== id);
                                        ...
                                        <div className="flex items-center gap-3 min-w-0">
                                            <div>
                                                  <div className="text-xs text-muted-foreground">
                                                      ...
                                                      {match.courtName && <span> • {match.courtName}{match.courtLocation ? ` - ${match.courtLocation}` : ''}</span>}
                                                  </div>
