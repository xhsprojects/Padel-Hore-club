...
import type { Season, WithId, UserProfile, Match, FinalLeaderboardPlayer, Tier, TierThresholds } from '@/lib/types';
...
export function SeasonManagement({ onEdit, onDelete }: SeasonManagementProps) {
  ...
  const getRetentionRate = (tier: Tier): number => {
    switch (tier) {
        case 'gold': return 0.35;
        case 'silver': return 0.30;
        case 'bronze': return 0.25;
        case 'lower bronze': return 0.20;
  ...
  const confirmActivation = async () => {
    ...
    const tierSettingsDoc = await getDoc(doc(firestore, 'settings', 'tier_thresholds'));
    const thresholds = tierSettingsDoc.exists() ? tierSettingsDoc.data() as TierThresholds : DEFAULT_THRESHOLDS;
    ...
    usersSnapshot.forEach(userDoc => {
        ...
        const retentionRate = getRetentionRate(playerTier);
